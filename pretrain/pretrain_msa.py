...
def get_batch(data_iterator):
    ...
    # msa_aligns = len(tokens)
    # msa_length = len(tokens[0])
    # attention_mask = torch.zeros((msa_aligns, msa_length), dtype=torch.bool)
    # position_ids = torch.arange(0, msa_length).repeat(msa_aligns, 1)
