...
def build_training_sample(sample,
    ...
    # We assume that we have at only one MSA in the sample
    assert len(sample) == 1, 'only support batch size = 1'
    sample = sample[0]
    args = get_args()
    ...
    # msa_length = min(raw_length, max_token_num // msa_aligns)
    # if msa_aligns == 1:
    #     print('MSA with only one sequence')
    ...
    for s in msa_sample:
        ...
        tokens += s.tolist()
    # tokens = msa_sample.reshape(-1).tolist()
